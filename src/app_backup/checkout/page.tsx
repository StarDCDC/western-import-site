...
export default function CheckoutPage() {
  ...
  // Calculate shipping
  let shippingCost = 0;
  if (form.deliveryMethod === 'COURIER_CHISINAU') {
    shippingCost = subtotal >= 3000 ? 0 : 150;
  } else if (form.deliveryMethod === 'COURIER_NATIONAL') {
  ...
  const freeShippingThreshold = form.deliveryMethod === 'COURIER_NATIONAL' ? 5000 : 3000;
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {[
                      ...
                      { id: 'COURIER_CHISINAU' as const, label: 'Curier Chișinău', desc: `${subtotal >= 3000 ? 'Gratuit' : '150 MDL'} • 1-2 zile`, icon: Truck },
                      { id: 'COURIER_NATIONAL' as const, label: 'Curier național', desc: `${subtotal >= 5000 ? 'Gratuit' : '250 MDL'} • 2-5 zile`, icon: Package },
                    ].map((m) => {
                      const Icon = m.icon;
                      return (
                        <button
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div data-error={errors.address ? 'true' : undefined}>
                        <label className="block text-xs font-medium text-slate-500 mb-1">Adresa *</label>
                        <input
                          type="text"
                          value={form.address}
                          onChange={(e) => updateField('address', e.target.value)}
                          className={`w-full py-2.5 px-3 rounded-lg border ${errors.address ? 'border-red-400 bg-red-50 dark:bg-red-950/20' : 'border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700'} text-sm`}
